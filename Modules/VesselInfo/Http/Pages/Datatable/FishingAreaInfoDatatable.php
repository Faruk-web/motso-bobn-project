...
class FishingAreaInfoDatatable extends DataTableComponent
{
    protected $index = 0;
    ...
    public function columns(): array
    {
        return [
           Column::make('SN','id')->format(fn () => ++$this->index +  ($this->page - 1) * $this->perPage),
            Column::make('Area Name', 'name')->searchable()->sortable(),
                ...
                ->buttons([
                    LinkColumn::make(' Edit')
                        ...
                        ->attributes(function ($row) {
                            return [
                                'data-id' => $row->id,
                                'data-listener' => 'openFishingAreaInfoModal',
                        ...
                        // ->title(fn ($row) => 'Print')
                        // ->location(fn ($row) =>route('vesselinfo.fishing.mpdf.area',['id' => $row->id]))
                        // ->attributes(function ($row) {
                        //     return [
                        //         'data-id' => $row->id,
                        //         'data-listener' => '',
                    ...
                    LinkColumn::make(' Delete')
                        ...
                        ->attributes(function ($row) {
                            return [
                                'data-id' => $row->id,
                                'data-listener' => 'fishingAreaInfoDelete',
    ...
    {
        return FishingAreaInfo::select('id')->orderBy('name', 'asc'); // Select some things
    }
}
