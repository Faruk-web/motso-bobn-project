...
class VesselInfoListDatatable extends DataTableComponent
{
    protected $index = 0;
   ...
   public function bulkActions(): array
   {
            ...
            ->buttons([
                ...
                ->attributes(function ($row) {
                    return [
                        'data-id' => $row->id,
                        'data-listener' => 'openVesselInfoListModal',
                        'class' => 'btn btn-secondary callEvent ',
                        'icon' => 'fa fa-print',
                    ];
                }),
                ...
                ->attributes(function ($row) {
                    return [
                        'data-id' => $row->id,
                        'data-listener' => 'vessellistInfoDeleteModal',
                        'class' => 'btn btn-danger callEvent',
                        'icon' => 'fa fa-trash',
                    ];
                }),
                LinkColumn::make(' Approve')
                ->title(fn ($row) => 'Approve')
                ->location(fn ($row) => 'javascript:void(0)')
                ->attributes(function ($row) {
                    return [
                        'data-id' => $row->id,
                        'data-listener' => 'vessellistInfoModal',
    ...
    public function builder(): Builder
    {
        ...
        // return VesselInfo::with('OfficeInfo')->where('status','2')->where('branch_id',$id); 

        $offfice_id = auth()->user()->branch_id;
        $auth_name = auth()->user()->name;
        if($auth_name== "Super Admin"){
            return VesselInfo::where('status','2');
        }else{
            return VesselInfo::where('status','2')->where('branch_id',$offfice_id)->orderBy('name', 'asc'); // Select some things
        }
    }
}
