...
@endphp
<div class="form-group mt-2" style="color: #5845bb;font-size: 16px;">
    {{-- @if ($attributes['label'])
    ...
    <div class="form-control-wrap" wire:key="{{ $jsId }}">
        <div wire:ignore>
            <select id="{{ $jsId }}" @isset($attributes['multiple']) multiple @endisset {{ $attributes->wire('model') }}
                ...
                @foreach ($VesselSetupStatus as $key => $value)
                <option value="{{ $value->id }}">{{ $show_type == "name" ? $value->name : $value->code }}</option>
