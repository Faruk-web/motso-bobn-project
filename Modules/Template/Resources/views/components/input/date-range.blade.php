<div class="" style="font-size: 16px;">
    ...
    <div wire:ignore>
        <input id="{{ $attributes->wire('model')->value }}" @if ($attributes['read-only']) readonly @endif
    ...
    <script>
        $(document).ready(function() {
            $('#{{ $attributes->wire('model')->value }}').daterangepicker({
                ...
                @else
                    @this.set("{{ $attributes['wire:model'] }}", start.format('DD-MM-YYYY hh:mm A') +
                        ' to ' + end.format('DD-MM-YYYY hh:mm A'));
                @endisset
            });
        });
    </script>
