<div wire:ignore class="mt-2" style="font-size: 16px;">
    <label for="{{ $attributes->wire('model')->value }}">{{ $attributes['label'] }}
        @isset($attributes['require'])
            <span class="text-danger">(*)</span>
        @endisset
    </label>
    <textarea id="{{ $attributes->wire('model')->value }}" {{ $attributes->wire('model') }}
...
@push('scripts')
    ...
    <script>
        ...
        window.addEventListener('editorUpdate', event => {
            $("#{{ $attributes->wire('model')->value }}").summernote("code", $(
                "#{{ $attributes->wire('model')->value }}").val());
        });
    </script>
@endpush
